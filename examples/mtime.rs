...
fn main() -> ! {
    ...
    rcc.iopenr.write(|w| w.iopaen().set_bit().iopcen().set_bit());
    ...
    struct Debouncer {
        state: stm32g071::gpioc::idr::IDR0_A,
        count: u32,
    }
    let mut d = Debouncer {
        state: stm32g071::gpioc::idr::IDR0_A::High,
        count: 0,
    };
    ...
    loop {
         ...
         */
        d.count = 0;
        if d.state == stm32g071::gpioc::idr::IDR0_A::Low {
            gpioa.odr.write(|w| w.odr5().low());
            d.state = stm32g071::gpioc::idr::IDR0_A::High;
        } else {
            // Print mtime on rising edge
            gpioa.odr.write(|w| w.odr5().high());
            dbg!(mtime().unwrap());
            d.state = stm32g071::gpioc::idr::IDR0_A::Low;
        };
    }
}
